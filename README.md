...
| **Embedding** | `huggingface` (bge-m3 etc.), `google` (Gemini) | `indexing.embedding.type` |
...
| **Reranking** | `none` (passthrough) | `query.reranking.type` |
| **Generation** | `ollama` (local), `edenai` (cloud gateway), `google` (Gemini) | `query.generation.type` |
...
```
data/sources/knowledge_base.pdf          # or student_handbook.pdf, etc.
data/datasets/testKB_dataset.jsonl       # see data/datasets/README.md for format
```

If using Ollama, pull the model specified in `configs/base.yaml`:
...
```yaml
# configs/experiments/hf_chroma_baseline.yaml
extends: ../base.yaml

name: "hf_chroma_baseline"
description: "Local baseline with Chroma instead of FAISS"
...
# or with Make:
make experiment CONFIG=configs/experiments/hf_chroma_baseline.yaml
```
...
```bash
python scripts/compare.py results/baseline results/hf_chroma_baseline
...
| Chunk size | `indexing.chunking.params.chunk_size` | 512 |
| Chunk overlap | `indexing.chunking.params.chunk_overlap` | 50 |
| Embedding model | `indexing.embedding.type` + `.params.model_name` | `huggingface` / `BAAI/bge-m3` |
| Vectorstore | `indexing.vectorstore.type` | `faiss` |
...
| Parameter | Config Path | Default |
|-----------|------------|---------|
| Query transform | `query.query_transform.type` | `passthrough` |
...
| Generator | `query.generation.type` | `ollama` |
| LLM model | `query.generation_llm.model_name` | `qwen3:14b` |
| System prompt | `query.prompt.system_template` | Academic advisor prompt |
...
| Parameter | Config Path | Default |
|-----------|------------|---------|
| Dataset | `evaluation.dataset` | `data/datasets/testKB_dataset.jsonl` |
| Mode | `evaluation.mode` | `full` (or `retrieval_only`) |
| Number of runs | `evaluation.num_runs` | 3 |
| Evaluator LLM | `evaluation.evaluator_llm` | Ollama qwen3:14b |
| Evaluator embedding | `evaluation.evaluator_embedding` | HuggingFace bge-m3 |
...
| Config | Purpose |
|--------|---------|
| `v1_replication.yaml` | Reproduce the ACMSE paper's Retrieval-based ARGObot (Gemini 1.0 Pro + ChromaDB + Google embeddings) |

### Single-Variable Isolation

Each changes exactly one thing from the baseline:

| Config | Variable Changed |
|--------|-----------------|
| `hf_chroma_baseline.yaml` | Vectorstore: FAISS → Chroma |
| `recursive_custom_baseline.yaml` | Chunker: LangChain → custom implementation |
| `edenai_baseline.yaml` | Generator: Ollama → Eden AI (GPT-4) |
| `hf_faiss_contextualizer.yaml` | Query transform: passthrough → LLM contextualizer |

### Retrieval-Only (no generation, fast iteration)

| Config | What It Tests |
|--------|--------------|
| `retrieval_only_hf_faiss.yaml` | Baseline retrieval quality (FAISS) |
| `retrieval_only_hf_chroma.yaml` | Baseline retrieval quality (Chroma) |
| `google_chroma_retrieval_only.yaml` | Google embedding retrieval quality |

### Full-Stack Alternatives

| Config | Stack |
|--------|-------|
| `google_chroma_contextualizer.yaml` | All-Google: Gemini embeddings + Chroma + Gemini contextualizer + Gemini generation |
| `edenai_chroma_contextualizer.yaml` | Cloud: Eden AI generation + Chroma + Eden AI contextualizer |
| `hf_chroma_contextualizer.yaml` | Local: bge-m3 + Chroma + Ollama contextualizer |
| `recursive_custom_chroma_contextualizer.yaml` | Local: custom chunker + Chroma + Ollama contextualizer |

### Evaluator Variants

| Config | What It Tests |
|--------|--------------|
| `edenai_llm_judge.yaml` | Eden AI (GPT-4) as both generator and evaluator judge |
...
| RAGAS timeout on local Ollama | Increase `evaluation.run_config.timeout` (default: 1200s) or reduce `evaluation.run_config.max_workers`. |
